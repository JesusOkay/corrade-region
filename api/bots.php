...
// Guardar el índice en un archivo para continuar desde donde se quedó
$index_file = '/tmp/last_index.txt';

// Comprobar si el archivo existe
if (file_exists($index_file)) {
    // Leer el índice guardado
    $index = (int) file_get_contents($index_file);
} else {
    // Si no existe, empezar desde 0
    $index = 0;
}
...
$next_index = $index + 1;
if ($next_index >= count($list)) {
    $next_index = 0;  // Si se llega al final de la lista, volver al principio
}
